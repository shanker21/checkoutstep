...
class QuoteManagement extends \Magento\Quote\Model\QuoteManagement
{
    ...
    {
        $writer = new \Zend_Log_Writer_Stream(BP . '/var/log/WOCPH_315_deliverydate_missing.log');
        $logger = new \Zend_Log();
        ...
        try {
            ...
            $logger->info('quoteroundid:'.$quote->getRoundId());
            $logger->info('quotedeliverydate:'.$deliveryDate );
            ...
            $order->setData('round_id', $quote->getRoundId());
            $order->setData('vendor_id', $quote->getVendorId());
            $order->setData('delivery_available_id', $quote->getDeliveryAvailableId());
            $order->setData('special_instructions', $quote->getSpecialInstructions());

            $logger->info('orderdeliverydate:'.$order->getDeliveryDate());
            $logger->info('orderroundid:'. $order->getRoundId());
            $requestBrochure = $quote->getRequestBrochure();
            if ($requestBrochure) {
                $order->setRequestBrochure($requestBrochure);
            }
